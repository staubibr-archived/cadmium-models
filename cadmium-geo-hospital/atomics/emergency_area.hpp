...
#include <cadmium/web/web_tools.hpp>
...
struct EmergencyArea_defs{
    struct rejected_1 : public cadmium::web::web_in_port<Emergency_t> {};
    struct out_1 : public cadmium::web::web_out_port<Emergency_t> {};
    struct out_2 : public cadmium::web::web_out_port<Emergency_t> {};
    struct out_3 : public cadmium::web::web_out_port<Emergency_t> {};
};

// model parameters
struct EmergencyArea_params {
	std::string id = ""; 		// geo area id
	// int id = 1; 				// geo area id
...
class EmergencyArea{
    ...
    static EmergencyArea_params params_from_feature(json j) {
		...
		p.id = j.at("properties").at("dauid").get<std::string>();
		p.population = j.at("properties").at("DApop_2016").get<int>();
		p.n_ports = 3;
		p.emergency_max = tools::round_to_int(rate * p.population);
    ...
    typename make_message_bags<output_ports>::type output() const {
		...
		if (state.port_i == 1) std::get<message_bag<typename EmergencyArea_defs::out_1>>(bags).messages.push_back(em);
		else if (state.port_i == 2) std::get<message_bag<typename EmergencyArea_defs::out_2>>(bags).messages.push_back(em);
		else if (state.port_i == 3) std::get<message_bag<typename EmergencyArea_defs::out_3>>(bags).messages.push_back(em);
